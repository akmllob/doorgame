...
function iHeartYou(){
	...
	if(window.location.hash){
		...
		vtext.textContent = "this for you rangga, happy valentine day";
	}
...
function socialShare(event,type){
	...
	switch(type){
		case "facebook":
			...
			url += "&t="+encodeURIComponent("A lovely message for rangga. This minigame only takes a minute to play, check it out! it's a(door)able --");
			width = 626;
			height = 436;
			break;
		case "twitter":
			url += "https://twitter.com/share?url="+encodeURIComponent(link);
			url += "&text="+encodeURIComponent("A lovely message for rangga, in this 1-min minigame. http://pic.twitter.com/DK5vnPzEVn"); // add twitter pic.
			url += "&via=ncasenmare";
			width = 640;
			height = 400;
			break;
		...
		case "tumblr":
			...
			url += "&description="+encodeURIComponent("A lovely message for hanim, in this 1-min minigame.");
			width = 446;
			height = 430;
			break;
		case "reddit":
